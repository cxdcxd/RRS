...
MoveitTool::MoveitTool(ros::NodeHandle n):nh(n)
{
  // initialization
  // robot model
  //ROS_INFO("Start moveittool instance initilization111");
  ...
  global_frame = "origin";
  // end effector link
  eef_link = robot_model->getLinkModel("panda_link8");
...
//  link_model_ptrs = robot_model->getLinkModelsWithCollisionGeometry();
  ...
  //build a  cylinder collision object for robot link76 
  link_shape76 = std::make_shared<shapes::Cylinder>(0.1,0.2);
  std::shared_ptr<fcl::CollisionGeometryd> link_geometry;
  buildGeometry(link_shape76,link_geometry);
...
//  std::cout <<"Construct collision object for link7 and link6 " << std::endl;
  link_shape54 = std::make_shared<shapes::Cylinder>(0.1,0.35);
  buildGeometry(link_shape54,link_geometry);
...
//  std::cout <<"Construct collision object for link5 and link4 " << std::endl;
  ...
  link_shape32 = std::make_shared<shapes::Cylinder>(0.1,0.3);
...
//  auto t1 = std::chrono::high_resolution_clock::now();
  ...
  if (update_co)
  {
      auto link_transform7 = current_state->getGlobalLinkTransform("panda_link8");
...
//     obstacle_marker.color.g = 1.0;
     ...
     //shapes::constructMarkerFromShape(link_shape76.get(),obstacle_marker);
     //obstacle_marker.ns="collision76";
     //marker_publisher.publish(obstacle_marker);

      auto link_transform2 = current_state->getGlobalLinkTransform("panda_link2");
      const auto& cylinder2_translation = link_transform2.translation();
      const auto& cylinder2_rotation = link_transform2.rotation();
      crobot_ptrs[2]->setTransform(cylinder2_rotation,cylinder2_translation);
     //tf::poseEigenToMsg(link_transform2,obstacle_marker.pose);
     //shapes::constructMarkerFromShape(link_shape32.get(),obstacle_marker);
     //obstacle_marker.ns="collision2";
     //marker_publisher.publish(obstacle_marker);

     auto link_transform4 = current_state->getGlobalLinkTransform("panda_link4");
      const auto& cylinder4_translation = link_transform4.translation();
      const auto& cylinder4_rotation = link_transform4.rotation();
      crobot_ptrs[2]->setTransform(cylinder4_rotation,cylinder4_translation);
     //tf::poseEigenToMsg(link_transform4,obstacle_marker.pose);
     //shapes::constructMarkerFromShape(link_shape32.get(),obstacle_marker);
     //obstacle_marker.ns="collision4";
     //marker_publisher.publish(obstacle_marker);

      auto link_transform5 = current_state->getGlobalLinkTransform("panda_link5");
      const auto& cylinder5_translation = link_transform5.translation();
      const auto& cylinder5_rotation = link_transform5.rotation();
      crobot_ptrs[1]->setTransform(cylinder5_rotation,cylinder5_translation);
     //tf::poseEigenToMsg(link_transform5,obstacle_marker.pose);
     //shapes::constructMarkerFromShape(link_shape54.get(),obstacle_marker);
     //obstacle_marker.ns="collision54";
     //marker_publisher.publish(obstacle_marker);


      auto link_transform3 = current_state->getGlobalLinkTransform("panda_link3");
