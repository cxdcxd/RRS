...
NmpcNlopt::NmpcNlopt(ros::NodeHandle nh):MoveitTool(nh)
{
  // load parameters
  bool param_ok=true;
  if(!nh.getParam("nlopt_mpc_main_node/panda/panda_inv_pid/p",p))
  {
    ROS_ERROR("error loading  p param");
    param_ok = false;
  }
  if(!nh.getParam("nlopt_mpc_main_node/panda/panda_inv_pid/i",i))
  {
    ROS_ERROR("error loading  i param");
    param_ok = false;
  }
  if(!nh.getParam("nlopt_mpc_main_node/panda/panda_inv_pid/d",d))
  {
...
//  }
  ...
  joint_state_sub = nh.subscribe("/panda_simulator/custom_franka_state_controller/joint_states", 100, &NmpcNlopt::jointStateSubCB,this);
  ...
  goal_pub = nh.advertise<franka_core_msgs::JointCommand>("/panda_simulator/motion_controller/arm/joint_commands",1);
...
//  std::fill(joint_velocities.begin(),joint_velocities.end(),0);
  ...
  //goal_position << 0.4328791,0.0870096,0.8912825, -0.0070424,0.9941063,0.0636445,0.1202173;
  // simulation 2
...
//  fout3.open(filename3);
  ...
  // If there is no other node like gazebo subscribe topic msg published by goal_pub, then wait
   ...
   while(goal_pub.getNumSubscribers() < 1){
     ROS_INFO("No subscribers, Going to sleep");
    sleep_t.sleep();}
...
{
  double uplimit[7] = {2.8973, 1.7628, 2.8973, -0.0698, 2.8973, 3.7525, 2.8973};
  double lowlimit[7] = {-2.8973, -1.7628, -2.8973, -3.0718, -2.8973, -0.0175, -2.8973};
 ...
 // }
  ...
  if(!state_sub_started)
  {
    ...
    for(int i=0;i<joint_num;i++)
    {
      ...
      std::cout << msg->position[i+2]<<std::endl;
...
//  std::vector<double> goal_q(7);
  ...
  for(int i=0;i<7;i++)
  {
    //std::cout <<"Joint velocity " << i << ":  " << u[i] << std::endl;
