    ...
    public class Network<PUBTYPE, SUBTYPE>
            {
                        ...
                        if (request != null)
                        {
                            try
                            {
                                ...
                                if (dresult)
                                {
                                    ...
                                    RobolandConnection result = Serializer.Deserialize<RobolandConnection>(ms);
                ...
                public void sendRequest(string req_msg)
                {
                    ...
                    if (state == NetworkState.CONNECTING || state == NetworkState.CONNECTED)
                    {
                        try
                        {
                            RobolandConnection cmd = new RobolandConnection();
                            cmd.ip = local_ip;
                            cmd.status = req_msg;

                            System.IO.MemoryStream ms = new System.IO.MemoryStream();
                            Serializer.Serialize<RobolandConnection>(ms, cmd);
                            Msg msg = new Msg();
