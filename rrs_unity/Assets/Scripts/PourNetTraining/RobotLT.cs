...
/// </summary>
public class RobotLT : MonoBehaviour {
    ...
    public SolidLT getHeldObject()
    {
        return this.heldObject;
    }

    public void setHeldObject(SolidLT heldObject)
    {
    ...
    public void createRobot(GameObject prefab, PhysicMaterial material=null, bool addCollider=false) {
        ...
        if (addCollider) {
            ...
            this.getRobotHand().AddComponent<RigidBodyCollisionLT>();
            this.getRobotHand().GetComponent<RigidBodyCollisionLT>().robot = this;
        }
    ...
    public void holdObject(SolidLT solid, GameObject workspace, bool useJoint=false, bool useMeasurer=false, bool isTarget=false) {
        ...
        if (!isTarget)
        {
            ...
            solid.getSolidObject().transform.position = workspace.transform.position + new Vector3(0, 8 * workspaceBounds.extents.y, 0);
            solid.setChildOf(workspace);
        }
        ...
        if (useJoint) {
            ...
            if (useMeasurer) {
                ForceInformationLT fInfo = this.getRobotHand().AddComponent<ForceInformationLT>();
    ...
    public void remove() {
        Destroy(this.getRobotHand().GetComponent<ForceInformationLT>());
        Destroy(this);
    }
}